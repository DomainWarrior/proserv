  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-3xl text-navy-DEFAULT">Customers</h1>
            ...
            <tbody className="divide-y divide-[--gray-100]">
              {customers?.map(c => (
                <tr key={c.id} className="hover:bg-[--off-white] transition-colors">
                  <td className="px-4 py-3 font-medium text-navy-DEFAULT">{c.full_name ?? '—'}</td>
                  <td className="px-4 py-3 text-[--gray-500]">{c.id.slice(0, 8)}…</td>
                  <td className="px-4 py-3 text-[--gray-500]">{c.phone ?? '—'}</td>
                  <td className="px-4 py-3 font-semibold">{countMap[c.id] ?? 0}</td>
                  <td className="px-4 py-3 text-[--gray-500]">{c.city ? `${c.city}, ${c.state}` : '—'}</td>
